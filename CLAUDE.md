...
```bash
# Full deploy — prompts for stack name, optional custom domain, optional KB files directory
...
1. ACM certificate (us-east-1, Route53 DNS validation) — skipped if no custom domain
2. Build Lambda zips and upload to S3
3. Pre-flight check for orphaned retained resources (aborts with delete commands if found)
4. `aws cloudformation deploy`
5. Upload `index.html`, `favicon.ico`, `system_prompt.txt` to S3
6. Upload KB files and trigger Bedrock sync — skipped if no directory provided
7. Route53 CNAME pointing custom domain to CloudFront — skipped if no custom domain

## CloudFormation parameters

`parameters.json` stores all parameter values. `deploy.py` writes ARNs back to this file when it creates new resources. Keep this file committed so re-deploys don't recreate those resources.

`SENTINEL = "{GENERATED BY deploy.py AT RUNTIME}"` in any field means deploy.py will create or discover that resource. Setting `ChatbotAcmCertificateArn` to `SENTINEL` triggers a new cert request on next deploy.
...
| `LambdaCodeS3Bucket` | Auto-derived as `{AppName}-rag-lambdas` |
| `RagSignerSuffix` | Auto-generated 5-char hex UUID each deploy |
| `ChatbotAlias` | Custom domain (e.g. `foo.example.com`) or `""` |
| `ChatbotAcmCertificateArn` | ACM ARN, `""`, or `SENTINEL` |
...
Single data bucket: `{AppName}-rag-chatbot-data`
